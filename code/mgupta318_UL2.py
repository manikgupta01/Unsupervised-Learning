...
def plot_learning_curves_NN(title, train_losses, train_accuracies, valid_accuracies):
    ...
    plt.legend(loc="best")
    plt.savefig("UL2_MLP_Loss_Curve_" + title + ".png")
    ...
    plt.legend(loc="best")
    plt.savefig("UL2_MLP_Accuracy_Curve_" + title + ".png")
...
def main():
    ...
    t_bef = time.time()
    kmeans_pca = KMeans(n_clusters=3, random_state=RANDOM_STATE).fit(pca_trans_train)
    t_aft = time.time()
    ...
    ica = FastICA(n_components = 95, algorithm = 'parallel',whiten = True, random_state=RANDOM_STATE)
    ica.fit(X_train)
    ...
    t_bef = time.time()
    kmeans_ica = KMeans(n_clusters=3, random_state=RANDOM_STATE).fit(ica_trans_train)
    t_aft = time.time()
